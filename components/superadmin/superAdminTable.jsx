...
export default function SuperAdminTable() {
    ...
    try {
      ...
      if (
        selectedMembers.some(
          (member) => member?._id === id && member?.isFeatured
        )
      ) {
  ...
  const activeUser = async (id) => {
    try {
      ...
      if (
        selectedMembers.some(
          (member) => member?._id === id && member?.isVerified
        )
      ) {
  ...
  const filteredMembers = selectedMembers.filter(
    (member) =>
      ...
      (searchTerm === "" ||
        member.nameOftheFirm.toLowerCase().includes(searchTerm.toLowerCase()))
  );
        ...
        <table className="bg-primary text-black table">
          <thead aria-label="members-table" className="text-black text-lg">
            <tr>
              ...
              <th>Ads Status</th>
              <th>Address</th>
              {session?.user?.role === "superadmin" && session && (
            ...
            {filteredMembers.map((member, index) => (
              ...
              <tr key={member._id}>
                ...
                <td>
                  <div
                    className={`badge badge-outline ${
                      member?.isVerified ? "text-green-500" : "text-red-500"
                    }`}
                  >
                ...
                <td>
                  <div
                    className={`badge badge-outline ${
                      member?.isFeatured ? "text-green-500" : "text-red-500"
                    }`}
                  >
                ...
                <td className="">
                  <button
                    type="button"
                    className={`btn ${
                      member?.isFeatured ? "btn-error" : "btn-success"
                    }`}
                    onClick={() =>
                      member?.isFeatured
                        ? featuredUserRemove(member?._id)
                        : featuredUser(member?._id)
                    }
                  >
